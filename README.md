...
`USED` と `REMAIN` はAPIから取得した実際の割合です。`PACE` はウィンドウの経過時間と使用率から、現在のペースでリセットまで持つかを推定します。`EST. LEFT` と概算順位は、各プランへ仮の容量ポイントを割り当てて残量率を掛けた参考値です。各社が公開したトークン数や実行回数ではなく、実際のタスク内容によって大きく外れる可能性があります。
