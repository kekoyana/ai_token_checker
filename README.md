...
`EST. LEFT` は契約枠と残量率を掛けた概算残量です。公式倍率があるClaudeとagyは、Proを1とした「基準枠」で表示します。たとえばMax 20xの残量99%は `約19.8基準枠` です。倍率が公開されていないCodexは `Plus枠の96%` のように、そのプラン枠内での残量を表示します。プランを判定できなければ `算出不可` と表示します。
